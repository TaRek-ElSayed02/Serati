...
export default function AdminEdit({ initialData, onClose }: AdminEditProps) {
...
const updatePersonalInfo = async () => {
    ...
    await makeApiRequest(`${API_BASE_URL}/usercv/admin/${initialData.id}`, 'PATCH', data);
};
...
const manageSocialLinks = async () => {
    ...
    for (const link of socialLinks) {
        if (link.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/social/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                socialname: link.platform,
                usernamesocial: link.username || '',
            ...
            await makeApiRequest(`${API_BASE_URL}/social/admin/${link.id}`, 'PATCH', data);
        }
    }
};
...
const manageExperiences = async () => {
    ...
    for (const exp of experiences) {
        if (exp.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/experience/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                jobtitle: exp.jobTitle,
                companyname: exp.company,
            ...
            await makeApiRequest(`${API_BASE_URL}/experience/admin${exp.id}`, 'PATCH', data);
        }
    }
};
...
const manageEducation = async () => {
    ...
    for (const edu of education) {
        if (edu.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/education/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                degree: edu.degree,
                institution: edu.institution,
            ...
            await makeApiRequest(`${API_BASE_URL}/education/admin/${edu.id}`, 'PATCH', data);
        }
    }
};
...
const manageSkills = async () => {
    ...
    for (const skill of skills) {
        if (skill.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/skills/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                skillname: skill.name,
                level: skill.level,
            ...
            await makeApiRequest(`${API_BASE_URL}/skills/admin/${skill.id}`, 'PATCH', data);
        }
    }
};
...
const manageLanguages = async () => {
    ...
    for (const lang of languages) {
        if (lang.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/language/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                languagename: lang.name,
                level: lang.proficiency
            };
            await makeApiRequest(`${API_BASE_URL}/language/admin/${lang.id}`, 'PATCH', data);
        }
    }
};
...
const manageCertificates = async () => {
    ...
    for (const cert of certificates) {
        if (cert.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/certificate/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                certificatename: cert.name,
                link: cert.link || '',
            ...
            await makeApiRequest(`${API_BASE_URL}/certificate/admin/${cert.id}`, 'PATCH', data);
        }
    }
};
...
const manageProjects = async () => {
    ...
    for (const project of projects) {
        if (project.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/project/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                projectname: project.name,
                content: project.content
            };
            await makeApiRequest(`${API_BASE_URL}/project/admin/${project.id}`, 'PATCH', data);
        }
    }
};
...
const manageAdditionalSections = async () => {
    ...
    for (const section of customSections) {
        if (section.id.startsWith('new-')) {
            ...
            await makeApiRequest(`${API_BASE_URL}/additional/admin/${currentUsercvId}`, 'POST', data);
        } else {
            const data = {
                sectiontitle: section.title,
                addcontent: section.content,
            ...
            await makeApiRequest(`${API_BASE_URL}/additional/admin/${section.id}`, 'PATCH', data);
        }
    }
};

const deleteItem = async (type: string, id: string) => {
    await makeApiRequest(`${API_BASE_URL}/${type}/admin/${id}`, 'DELETE');
};
    ...
    const translations = {
        ar: {
            title: 'تعديل السيرة الذاتية (وضع المدير)',
                                            ...
                                            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <select value={lang.proficiency} onChange={(e) => updateLanguage(lang.id, 'proficiency', e.target.value)} className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#192A3D]">
                                                        <option value="begginer">{t.basic}</option>
                                                        <option value="intermediate">{t.conversational}</option>
                                                        <option value="advanced">{t.fluent}</option>
